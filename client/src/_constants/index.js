export const API_BASE_URL='http://localhost:8040/leave_system';
export const API_EMPLOYEE_BASE_URL='http://localhost:4001/employee';

// validation things below here
export const LEAVE_TYPE_MIN_LENGTH =4;
export const LEAVE_TYPE_MAX_LENGTH =20;
export const ROLE_MIN_LENGTH =2;
export const Role_MAX_LENGTH =20;
export const RECUITMENT_MIN_LENGTH =4;
export const RECUITMENT_MAX_LENGTH =20;
export const DESIGNATION_MIN_LENGTH =2;
export const DESIGNATION_MAX_LENGTH =20;